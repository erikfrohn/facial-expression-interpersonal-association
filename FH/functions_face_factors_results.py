...
def compute_results_factors(path_to_analysis_folder,path_to_result_folder, participant_id1, participant_id2):
 ...
 # Determine the CSV file name with markers
    ...
    csv_filename1 = f"pp{participant_id1}_{role1}_video_frames.csv"
    csv_filename2 = f"pp{participant_id2}_{role2}_video_frames.csv"
    ...
    for name in names:
        ...
        # Generate output CSV file path
        output_csv_path = os.path.join(path_to_factor_folder, f"pp{participant_id1}_pp{participant_id2}_{name}_result_factor.csv")
        ...
        for i in range(factors1.shape[1]):
            ...
            plot_filename = f'Factor Comparison_{label}_of_{name}_{participant_id1}_{participant_id2}.png'
