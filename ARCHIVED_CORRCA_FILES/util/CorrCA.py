...
def fit(X, version=2, gamma=0, k=None):
    ...
    temp = [np.nan_to_num(cov_matrix, nan=0.0, posinf=0.0, neginf=0.0) for cov_matrix in temp]
    ...
    #     print("Rw contains inf or NaN values. Cleaning matrix...")
    #     Rw = np.nan_to_num(Rw, nan=0.0, posinf=0.0, neginf=0.0)

    # # Compute Rt
    # Rt = N**2 * np.cov(np.nanmean(X, axis=0))
    # if not np.isfinite(Rt).all():
    #     print("Rt contains inf or NaN values. Cleaning matrix...")
    #     Rt = np.nan_to_num(Rt, nan=0.0, posinf=0.0, neginf=0.0)

    # # Compute Rb
    # Rb = (Rt - Rw) / (N-1)
    # if not np.isfinite(Rb).all():
    #     print("Rb contains inf or NaN values. Cleaning matrix...")
